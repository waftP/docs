...
{/* No imports — your bundler only allows /snippets/* imports */}

function StatusCheck({ url }) {
const [state, setState] = React.useState("idle"); // idle | loading | up | down
...
{state === "loading" ? "Checking…" : "Run status check"}
      ...
      <details style={{ marginTop: "0.75rem" }}>
        ...
        <ul>
          <li>
            If this page can’t reach the API from your browser, ensure the health endpoint allows cross-origin GETs (CORS):{" "}
            <code>Access-Control-Allow-Origin: *</code> (or your docs domain).
          </li>
