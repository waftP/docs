...
Obtain a Bearer token from the [Authentication API](/api-reference/auth/auth-token) using your `api_key` and `api_secret` (scoped to the **PAYMENTS** product).
...
Encode the resulting signature as <Tooltip tip="URL-safe base64 variant; uses - and _ instead of + and /">Base64URL</Tooltip>.
...
X-Idempotency-Key: `<transaction.reference>` <Tooltip tip="Client-provided key to dedupe retries on the same logical operation">?</Tooltip>
X-Custom-Signature: `<base64url_signature>`
```

> **Idempotency**: We guarantee at‑most‑once processing per `(client_id, X-Idempotency-Key)`. Retries with the same idempotency key return a rejected result.
...
- `originator.msisdn`: <Tooltip tip="International phone format; no leading +">E.164</Tooltip> without `+` (e.g., `2547…`).
