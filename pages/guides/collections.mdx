...
Collections represent **incoming customer payments** (e.g., paybill/till). The API is <Tooltip tip="You get 202 immediately; the final result arrives later via webhook">asynchronous</Tooltip> and uses the same auth/signing model as payouts.
...
Headers are identical to payouts (see that guide). Use `X-Idempotency-Key: <transaction.reference>` <Tooltip tip="Dedupes retries for the same collection request">?</Tooltip>.
...
- `msisdn` should be in <Tooltip tip="International phone format; no leading +">E.164</Tooltip> without the `+`.
