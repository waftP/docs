...
Transformed JSON sent via HTTP `POST` to `callback_url`. See `/pages/webhooks/payload` for full schema; example:
...
Status codes are consistent with `/pages/webhooks/payload` (`100` CREATED, `101` PENDING, `102` SUCCESS, `103` FAILED, `104` ESCALATED, `105` EXPIRED, `106` REFUNDED, `107` REVERSED).
...
- Included in `results.validation_hash` so you can recompute and verify. See `/pages/webhooks/verify-signature` for verification snippets and stronger HMAC guidance.
