...
#### Where to go next
- See `/pages/webhooks/event-types` for the status/code matrix.
- See `/pages/webhooks/payload` for the exact JSON shape.
- See `/pages/webhooks/retries-and-idempotency` for retry rules and deduping.
- See `/pages/webhooks/consumer-service` for the internal queue → delivery flow.
