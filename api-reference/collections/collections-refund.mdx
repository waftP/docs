...
description: "When and how to request a refund for a collection, plus status checks."
api: "POST /payments-api-service/v1/collections/refunds"
---
...
- **Duplicate charge** — same payer charged twice for the same order.
- **Wrong amount** — charged amount exceeds intended amount.
- **Customer dispute / cancellation** — service not delivered or order cancelled.
- **Compliance / regulatory** — mandated reversals by operator/bank.
- **Operational error** — beneficiary set incorrectly (wrong MSISDN/account).
